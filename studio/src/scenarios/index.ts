...
export interface ScenarioResult { id: string; title: string; group: 'navigation' | 'vision'; method: string; pass: boolean; metrics: ScenarioMetric[]; notes: string[]; durationMs: number }
export interface Scenario {
  id: string;
  title: string;
  group: 'navigation' | 'vision';
...
export const ALL_SCENARIOS: Scenario[] = [...NAV_SCENARIOS, ...VISION_SCENARIOS];
...
export function scenarioReportMarkdown(results: ScenarioResult[], opts: { generatedAt?: string } = {}): string {
  ...
  for (const group of ['navigation', 'vision'] as const) {
    lines.push(`### ${group === 'navigation' ? 'Navigation & localization' : 'Machine vision'}`, '');
